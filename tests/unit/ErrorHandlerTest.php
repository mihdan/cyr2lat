 ...
 * @group error-handler
 */
class ErrorHandlerTest extends CyrToLatTestCase {
	...
	public function test_filter_doing_it_wrong_trigger_error(): void {
		...
		// Not proper function.
		self::assertTrue( $subject->filter_doing_it_wrong_trigger_error( true, 'some_function', 'some_message<code>cyr2lat</code>more_text', 'some_version' ) );
		...
		self::assertFalse( $subject->filter_doing_it_wrong_trigger_error( true, '_load_textdomain_just_in_time', 'some_message<code>cyr2lat</code>more_text', 'some_version' ) );
	}
}
