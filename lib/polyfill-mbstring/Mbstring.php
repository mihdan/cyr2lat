	...
	public static function mb_convert_variables( $toEncoding, $fromEncoding, &$a = null, &$b = null, &$c = null, &$d = null, &$e = null, &$f = null ) {
		...
		\array_walk_recursive(
			...
			function ( &$v ) use ( &$ok, $toEncoding, $fromEncoding ) {
				if ( \false === ( $v = \Cyr_To_Lat\Symfony\Polyfill\Mbstring\Mbstring::mb_convert_encoding( $v, $toEncoding, $fromEncoding ) ) ) {
					$ok = \false;
				}
			}
		);
			...
			function ( array $m ) use ( $cnt, $convmap ) {
				...
				for ( $i = 0; $i < $cnt; $i += 4 ) {
					if ( $c >= $convmap[ $i ] && $c <= $convmap[ $i + 1 ] ) {
						return \Cyr_To_Lat\Symfony\Polyfill\Mbstring\Mbstring::mb_chr( $c - $convmap[ $i + 2 ] );
					}
				}
				return $m[0];
			},
